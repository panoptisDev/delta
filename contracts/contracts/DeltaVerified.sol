...
contract DeltaVerifiedStorage is Exponential {
...
contract DeltaVerified is DeltaVerifiedStorage, SafeToken {
    /**
     ...
     */
    function setRewardControlAddress(address rewardControl_) public returns(bool) {
     ...
     */
    function _supportMarket(address asset, address interestRateModel) external returns(bool) {
     ...
     */
    function supply(address asset, uint256 amount) public nonReentrant nonPaused onlyCustomerWithKYC returns(bool) {
        require(asset != address(0), "Asset can't be zero address");

        refreshDltSupplyIndex(asset, msg.sender);
    ...
    function withdraw(
        ...
        if(requestedAmount == MAX_UINT256) {
            local.withdrawCapacity = getAssetAmountForValue(
                asset,
                local.accountLiquidity
            );
    ...
    function repayBorrow(
        address asset, 
        uint256 amount
    ) public nonReentrant nonPaused onlyCustomerWithKYC returns(bool) {
        refreshDltBorrowIndex(asset, msg.sender);
    ...
    function liquidateBorrow(
        ...
        refreshDltSupplyIndex(assetCollateral, targetAccount);
        refreshDltSupplyIndex(assetCollateral, msg.sender);
        refreshDltBorrowIndex(assetBorrow, targetAccount);
    ...
    */
    ...
    function supplyOriginationFeeAsAdmin(
        ...
        refreshDltSupplyIndex(asset, admin);
    ...
    function refreshDltSupplyIndex(
        ...
        rewardControl.refreshDltSupplyIndex(market, supplier, false);
    }
    ...
    function refreshDltBorrowIndex(
        ...
        rewardControl.refreshDltBorrowIndex(market, borrower, false);
    }
}